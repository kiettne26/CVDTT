...
@section('title', 'Sửa thông tin món ăn')
        ...
        <div class="col-12">
            <div class="card shadow-lg border-0" style="border-radius: 1.5rem;">
                <div class="card-header bg-white border-bottom-0 rounded-top-4 px-4 py-3">
                    <div class="d-flex justify-content-between align-items-center">
                        <h3 class="mb-0 fw-bold text-primary">
                            <i class="fas fa-hamburger me-2 text-warning"></i>Sửa Món Ăn
                        </h3>
                ...
                <div class="card-body p-4">
                    <form action="{{ route('foods.update', $food->foodId) }}" method="POST" enctype="multipart/form-data" class="needs-validation" novalidate>
                        @csrf
                        @method('PUT')
                        <div class="row g-4">
                            <!-- Name -->
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" class="form-control shadow-none @error('name') is-invalid @enderror"
                                           id="name" name="name" value="{{ old('name', $food->name) }}"
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="mt-3 text-center">
                                    {{-- Ảnh preview cũ nếu có --}}
                                    @if($food->poster)
                                        <img id="imagePreview" src="{{ asset($food->poster) }}"
                                             alt="Ảnh món ăn" class="img-thumbnail"
                                             style="max-height: 180px; object-fit: cover; border-radius: 1.1rem;">
                                    @else
                            ...
                            <div class="col-12">
                                ...
                                <textarea class="form-control shadow-none @error('description') is-invalid @enderror"
                                          ...
                                          placeholder="Nhập mô tả món ăn" style="resize: vertical; border-radius: 14px;">{{ old('description', $food->description) }}</textarea>
                                @error('description')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
...
<script>
    function previewImage(input) {
        ...
        let noPreview = document.getElementById('noImagePreview');
